...
import dev.batuhanyetgin.mssecurityservice.client.CustomerClient;
...
public class AuthServiceImpl implements AuthService {
    ...
    public boolean isTokenExpired(String token) {
        return (tokenManager.isExpired(token) || customerClient.isCustomerExist(tokenManager.getSubject(token)));
    }

    @Override
    public LoginResponseDto login(LoginDto loginDto) throws AuthException {
        CustomerDto customerDto = modelMapper.map(customerClient.getByMail(loginDto.getEmail()), CustomerDto.class);
